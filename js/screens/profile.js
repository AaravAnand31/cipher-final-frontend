...
export async function renderProfile() {
      ...
      <div class="screen-body">
        ...
        <div class="profile-hero">
          <div style="display:flex;align-items:flex-end;justify-content:space-between">
            <div class="profile-avatar-ring" style="position:relative">
  ...
  // Click on connections stat → go to connections list
  document.getElementById('stat-connections').addEventListener('click', () => navigate('/connections'));

  // Load real stats async (so page renders immediately)
  loadProfileStats();
}
  ...
  try {
    ...
    if (!conns.length) {
      body.innerHTML = `
        <div class="empty-state">
          ...
          <button class="btn btn-primary" style="margin-top:20px;width:auto;padding:10px 24px"
            id="go-discover">Go to Discover</button>
        </div>`;
    ...
    body.innerHTML = `
      ...
      <div style="background:var(--bg-card);margin:0 16px;border-radius:var(--r-lg);overflow:hidden;box-shadow:var(--shadow-sm)">
        ${conns.map((c, i) => {
          ...
          const lastSeenStr = u.lastSeen ? timeAgoFull(u.lastSeen) : null;
          ...
          return `
            <div style="display:flex;align-items:center;gap:12px;padding:14px 16px;
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="font-size:12px;color:${isOnline ? '#34c759' : 'var(--label-tertiary)'};margin-top:2px">
                  ${isOnline ? 'Online now' : lastSeenStr ? `Last seen ${lastSeenStr}` : 'Offline'}
                </div>
              </div>

              <!-- Chat button -->
              <button class="btn btn-secondary-fill btn-sm" data-chat="${c.connectionId}"
                data-user='${JSON.stringify({ _id: u._id, username: u.username || u.name, department: u.department, year: u.year, photoURL: u.photoURL })}'
    ...
    document.getElementById('conn-list-body').innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-title">Could not load connections</div>
      </div>`;
  }
}
  ...
  document.getElementById('app').innerHTML = `
    <div class="screen screen-enter">
      ...
      <div class="screen-body" style="background:var(--bg-secondary);padding:16px">
        ...
        <div class="list-group" style="margin-bottom:20px">
          ${rows.map((r, i) => `
            <div class="list-row" data-action="${r.action}"
...
export function renderEditProfile() {
  ...
  let draftPhoto = p.photoURL || '';
  let draftCover = p.coverURL || '';
      ...
      <div class="screen-body" style="background:var(--bg-secondary);padding-bottom:40px">
          ...
          <div style="position:relative;margin-top:-36px;padding:0 20px 16px;display:flex;align-items:flex-end;justify-content:space-between">
            <div style="position:relative">
              <div id="avatar-tap" style="width:72px;height:72px;border-radius:50%;border:3px solid var(--bg-card);
                overflow:hidden;background:var(--accent-bg);display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:var(--shadow-md)">
                ${draftPhoto
                  ? `<img src="${draftPhoto}" style="width:100%;height:100%;object-fit:cover" />`
                  : `<span style="font-size:26px;font-weight:600;color:var(--accent)">${initials}</span>`}
              </div>
              <div id="photo-btn" style="position:absolute;bottom:0;right:0;width:24px;height:24px;
        ...
        <div class="form-section" style="margin:8px 16px 16px">
          <div class="form-row" style="border-bottom:none;align-items:flex-start">
            <textarea id="field-bio" rows="3" maxlength="200" placeholder="Tell people who you are…"
        ...
        <div class="form-section" style="margin:8px 16px 16px">
          <div class="form-row" style="border-bottom:none;align-items:flex-start">
            <textarea id="field-ice" rows="2" placeholder="Something fun about you…"
  ...
  const coverTap   = document.getElementById('cover-tap');

  [avatarTap, photoBtn].forEach(el => el.onclick = () => photoInput.click());
  coverTap.onclick = () => coverInput.click();
  ...
  photoInput.onchange = () => {
    ...
    reader.onload = e => {
      ...
      avatarTap.innerHTML = `<img src="${draftPhoto}" style="width:100%;height:100%;object-fit:cover" />`;
  ...
  document.getElementById('remove-photo')?.addEventListener('click', () => {
    draftPhoto = '';
    avatarTap.innerHTML = `<span style="font-size:26px;font-weight:600;color:var(--accent)">${initials}</span>`;
    toast('Photo removed');
  });

  document.getElementById('interest-chips').onclick = e => {
    e.target.closest('[data-interest]')?.classList.toggle('selected');
  };
  document.getElementById('looking-chips').onclick = e => {
    e.target.closest('[data-looking]')?.classList.toggle('selected');
  };
  ...
  document.getElementById('app').innerHTML = `
    <div class="screen screen-enter">
      ...
      <div class="screen-body" id="blocked-area" style="background:var(--bg-secondary)">
        ${blockedList.length ? blockedHTML() : `
          <div class="empty-state">
            <div class="empty-icon">🚫</div>
...
function blockedHTML() {
  return `
    <div class="list-group" style="margin:16px">
      ${blockedList.map((u, i) => `
        <div style="display:flex;align-items:center;gap:12px;padding:14px 16px;${i===blockedList.length-1?'':'border-bottom:0.5px solid var(--separator)'}">
          ...
          <button class="btn btn-secondary-fill btn-sm" data-unblock="${u.uid}" style="width:auto;padding:7px 14px;font-size:13px">Unblock</button>
        </div>`).join('')}
    </div>`;
}
...
function bindBlocked() {
  document.querySelectorAll('[data-unblock]').forEach(btn =>
    btn.addEventListener('click', async () => {
      ...
      document.getElementById('blocked-area').innerHTML = blockedList.length ? blockedHTML() : `
        <div class="empty-state">
          <div class="empty-icon">🚫</div>
          <div class="empty-title">No blocked users</div>
        </div>`;
